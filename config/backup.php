...
use App\Tasks\Cleanup\Strategies\SimpleStrategy;
...
return [
    ...
    'backup' => [
         ...
         * the backups.
         */
        'name' => $backupName = sprintf('%s-backup', str()->slug(env('APP_NAME', 'backup'))),
     ...
     */
    'notifications' => [
        'channels' => $channels = explode(',', (string) env('BACKUP_NOTIFIABLE_CHANNEL', 'discord')),
        ...
        'notifications' => [
            ...
            BackupWasSuccessfulNotification::class => $channels,
            HealthyBackupWasFoundNotification::class => $channels,
            CleanupWasSuccessfulNotification::class => $channels,
        ],
    ...
    'cleanup' => [
        ...
        'simple_strategy' => [
            /**
             * The number of old backups to keep.
             *
             * Accepted values:
             *  - null: Disable cleanup entirely. No backups will be deleted.
             *  - 0: Retain only the newest backup. All older backups are deleted.
             *  - N (where N >= 1): Retain the newest backup plus N older backups.
             *               Any backups older than that will be deleted.
             *
             * Examples:
             *
             * - keep_old_backups_count = null   // Disable cleanup
             * - keep_old_backups_count = 0      // Keep only the newest backup
             * - keep_old_backups_count = 50     // Keep newest + 50 older backups
             */
            'keep_old_backups_count' => (int) env('BACKUP_KEEP_OLD_BACKUPS_COUNT', 50),
        ],
